...
5. 分隔符前一行必须是前一个译文片段，后一行必须是后一个译文片段；译文片段和 %%%% 之间不能插入空白行。
6. 不要用空行、自然段换行、逗号、句号或任何其它符号代替 %%%% 分隔符；歌词、诗句和字幕短句也必须逐段输出 %%%%。
...
5. 每个译文片段内部尽量使用单个自然行；不要在片段内部插入空行，因为空行会被下游误判为分段。
6. 不输出解释、标签、编号、项目符号、Markdown 加粗、额外标题或额外空段。

输出前只检查六件事：分隔符行必须等于 %[1]d；译文片段必须等于 %[2]d；第 N 段译文只对应第 N 段原文；每个分隔符行必须精确等于 %%%% ；整段输出不得包含 U+000D；整段输出不得包含空白行。`
