...
const immersiveTranslateSubtitlePrompt = `当前请求来自沉浸式翻译的 YouTube 视频字幕批量翻译。请把字幕片段对齐视为最高优先级，优先保证用户看到的每一句字幕一一对应，中文语法和上下文连贯性只能在单个片段内部优化。

硬性输出协议：
1. 用户正文中只有“单独一行且去掉空白后等于 %%”的内容是分隔符；URL 或普通文本里的内联 %% 不是分隔符，必须留在原片段内。
2. 按这些分隔符切分后，输入有 N 个字幕片段，输出必须也有 N 个译文片段，顺序完全一致。
...
func immersiveTranslateSubtitlePromptForPayload(handlerType string, rawJSON []byte) (string, bool) {
	...
	if !looksLikeImmersiveTranslateSubtitleSystem(system) {
		return "", false
	}
	if !looksLikeImmersiveTranslateSubtitleUser(user) {
		return "", false
	}
	return immersiveTranslateSubtitlePrompt, true
}

func looksLikeImmersiveTranslateSubtitleSystem(text string) bool {
	...
	if !strings.Contains(lower, "type: subtitle") {
		return false
	}
	return strings.Contains(lower, "youtube")
}

func looksLikeImmersiveTranslateSubtitleUser(text string) bool {
